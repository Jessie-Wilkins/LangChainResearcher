...
def test_run_can_create_a_file_with_list_in_the_path():
    writer = FixedWriteFileTool()
    item1 = Item(item="Chicken Eggs",
                 description="Eggs that come from chickens")
    item2 = Item(item="Duck Eggs", description="Eggs that come from ducks")
...
def test_run_can_create_file_with_summary_in_the_path():
    writer = FixedWriteFileTool()
    summary = Summary(file_path="output/eggs.txt", summary="Eggs can come from various kinds of birds. Some of the birds that can lay eggs are robbins, chickens, and ducks.")
    writer._run(summary.json())

    assert os.path.isfile(summary.file_path)   

    with open(summary.file_path, 'r') as f:
        contents = f.read()
    expected_contents = summary.summary
    assert contents == expected_contents
